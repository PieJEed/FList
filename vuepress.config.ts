...
export default defineUserConfig({
  ...
  title: 'LiShare',
  // 网站的简介，有助于搜索引擎收录
  description: 'LiShare - 不止分享好的开源仓库',
  ...
  theme: FileList([
    {
        mountPath: "/",
      analysis: githubReleasesFilesAnalysis({ user: "PieJEed", repository: "FList" }),
    ...
    //路径格式：Windows/优化/
        {
      // 挂载路径
      mountPath: "Windows/优化/“此电脑”第三方图标删除",
      // 文件解析器，这里使用githubReleasesFilesAnalysis,可以解析github的release文件
      analysis: githubReleasesFilesAnalysis({
        // 仓库所有者的用户名
        user: "Return-Log",
        // 仓库所有者的仓库名
        repository: "Drive-Icon-Manager"
      }),
      downProxy: cloudflarePagesDownProxy(),
    },
        {
      // 挂载路径
      mountPath: "Windows/优化/ShowDesktopOneMonitor｜解决 WIN+E 会关闭多显示器的窗口",
      // 文件解析器，这里使用githubReleasesFilesAnalysis,可以解析github的release文件
      analysis: githubReleasesFilesAnalysis({
        // 仓库所有者的用户名
        user: "ruzrobert",
    ...
    //路径格式：Windows/美化/
        {
      // 挂载路径
      mountPath: "Windows/美化/WinDynamicDesktop・MAC功能 壁纸随时间改变",
      // 文件解析器，这里使用githubReleasesFilesAnalysis,可以解析github的release文件
      analysis: githubReleasesFilesAnalysis({
        // 仓库所有者的用户名
        user: "t1m0thyj",
    ...
    //分割
      ...
      analysis: githubReleasesFilesAnalysis({
        ...
        {
      // 挂载路径
      mountPath: "/Windows优化/“此电脑”第三方图标删除",
      // 文件解析器，这里使用githubReleasesFilesAnalysis,可以解析github的release文件
      analysis: githubReleasesFilesAnalysis({
        // 仓库所有者的用户名
        user: "Return-Log",
      ...
      // 挂载路径
      mountPath: "/Windows优化/“此电脑”第三方图标删除",
      // 这里使用githubReleasesFilesAnalysis,解析github的release文件
      analysis: githubReleasesFilesAnalysis({
        user: "Return-Log",
        repository: "Drive-Icon-Manager"
      }),
